...
def harvest_results():
    ...
    for region in ["A", "B", "C"]:
        ...
        json.dump(
            harvest,
            open(
                "results/harvest.Region{region}.json".format(region=region),
                "w+",
            ),
            sort_keys=True,
            indent=2,
...
def plotit(
    ...
    f = ROOT.TFile("results/multiplex.root")
    ...
    plot.drawOneSigmaBand(f.Get(bandName))
    plot.drawExpected(f.Get(expName))
    plot.drawObserved(f.Get(obsName), title="Observed Limit")
